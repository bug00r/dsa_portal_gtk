...
static void
open_mapeditor_cb (GtkToolButton *toolbutton, gpointer data)
{
    ...
    if (stack) 
    {
        GtkWidget *mapeditor_widget = GTK_WIDGET(gtk_stack_get_child_by_name(stack,(const gchar*)"mapeditor"));
        g_message("found mapeditor %p", mapeditor_widget);
        if (mapeditor_widget == NULL) 
        {
            alveran_hgen_init_app(GTK_APPLICATION(g_application_get_default()));
            GtkWidget *mapeditor_widget = alveran_mapeditor_new();
            gtk_stack_add_titled (stack, mapeditor_widget, (const gchar*)"mapeditor", (const gchar*)"mapeditor");
            GtkStyleProvider *provider = GTK_STYLE_PROVIDER (gtk_css_provider_new ());
            gtk_css_provider_load_from_resource (GTK_CSS_PROVIDER (provider), "/de/bug0r/alveran/css/default.css");

            apply_css (GTK_WIDGET(mapeditor_widget), provider);
            alveran_hgen_widget_init(mapeditor_widget);
        }
        gtk_stack_set_visible_child_name(stack, (const gchar*)"mapeditor");
    }
...
alveran_app_window_class_init (AlveranAppWindowClass *class)
{
    ...
    gtk_widget_class_bind_template_callback_full (GTK_WIDGET_CLASS (class), "open_mapeditor_cb", G_CALLBACK(open_mapeditor_cb));
